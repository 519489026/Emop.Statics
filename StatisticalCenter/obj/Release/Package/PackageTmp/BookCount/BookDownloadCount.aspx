<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="BookDownloadCount.aspx.cs" Inherits="StatisticalCenter.BookCount.BookDownloadCount" %>
...
<head runat="server">
    ...
    <script type="text/javascript">
        ...
        function Export(type) {
            ...
            $("#frmExport").attr("src", "ajax.ashx?action=GetBookDownloadExportData&pressId=" + pressid + "&bookGuid=" + bookGUID + "&startDate=" + startTime + "&endDate=" + endTime + "");
        }
        ///图书下拉控件数据
        var vTempPress;
        // 获取下拉控件的数据
...
<body style="overflow:hidden">
    <form id="form1" runat="server" autocomplete="off">
        ...
        <div ng-app="mainApp" ng-controller="countController" style="width: 820px;overflow:auto">
            <div>
            ...
            <table style="margin-top: 15px; background-color: white">
                <tr>
                    ...
                    <td style="width: 70px; text-align: center">图书</td>
                    <td style="width: 70px; text-align: center">下载次数</td>
                    <td style="width: 70px; text-align: center">下载人数</td>
                ...
                <tr>
                    ...
                    <td style="width: 70px; text-align: center" id="downloadTimes"></td>
                    <td style="width: 70px; text-align: center" id="downloadPeople"></td>
            ...
            <div style="height: 360px">
                <table id="titletable" style="margin-top: 20px;">
                    <tr class="trTitle">
                        ...
                        <td style="width: 80px; text-align: center">下载次数</td>
                        <td style="width: 80px; text-align: center">下载人数</td>
                        <td style="width: 140px; text-align: center">注册用户下载人数</td>
                    </tr>
                </table>
                ...
                <table id="contenttable" style="display: none;line-height:30px">
                    <tr ng-repeat="itemx in resultList">
                        ...
                        <td style="width: 140px; text-align: center;overflow:hidden;white-space:nowrap;text-overflow:ellipsis" title="{{itemx.BookName}}">
                            <div style="width: 140px; text-align: center; overflow: hidden; white-space: nowrap; text-overflow: ellipsis">
                                {{itemx.BookName}}
                            </div>
                        </td>
                        <td style="width: 140px; text-align: center;overflow:hidden;white-space:nowrap;text-overflow:ellipsis" title="{{itemx.PressName}}">
                            <div style="width: 140px; text-align: center; overflow: hidden; white-space: nowrap; text-overflow: ellipsis">
                                {{itemx.PressName}}
                            </div>
                        </td>
                        <td style="width: 80px; text-align: center">{{itemx.AllCount}}</td>
                        <td style="width: 80px; text-align: center">{{itemx.PersonCount}}</td>
                        <td style="width: 140px; text-align: center">{{itemx.RegistePersonCount}}</td>
                    </tr>
                </table>
            </div>
            <div style="clear: both; height: 10px;"></div>
    ...
    <script>
        ...
        angular.module('mainApp', []).controller('countController', function ($http, $scope) {
            $http.get('ajax.ashx?action=GetUserPressTable').success(function (data) {
                $http.get('ajax.ashx?action=GetBookDownloadCountData&pressId=' + pressid + '&bookGuid=' + bookGuid + '&startDate=' + start + '&endDate=' + end + '&pageIndex=' + index + '&pageSize=10').success(function (result) {
                    $scope.resultList = result.list;
                    $scope.dataList = data;
                    $("#downloadTimes").html(result.totalDownLoadCount);
                    $("#downloadPeople").html(result.totalPersonCount);
